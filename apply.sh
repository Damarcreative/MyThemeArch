...
SETTINGS_FILE="$HOME/.config/openbox/themes/$THEME/settings.conf"
...
# Apply bottom bar visibility setting
if [[ -f "$SETTINGS_FILE" ]]; then
    source "$SETTINGS_FILE"
    if [[ "$SHOW_BOTTOM_BAR" == "false" ]]; then
        # Remove pinsettings from polybar
        sed -i 's/battery dot pinsettings sysmenu/battery dot sysmenu/' ${PATH_PBAR}/config.ini
        # Kill tint2 if running
        killall -q tint2
    else
        # Ensure pinsettings in polybar
        if ! grep -q "pinsettings sysmenu" ${PATH_PBAR}/config.ini; then
            sed -i 's/battery dot sysmenu/battery dot pinsettings sysmenu/' ${PATH_PBAR}/config.ini
        fi
    fi
fi
